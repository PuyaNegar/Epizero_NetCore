            ...
            var thread = new Thread(() =>
            {
                try
                {
                    using (var component = new CourseStudentsSmsComponent())
                    {
                        ...
                        var studentUsers = component.Operation(courseStudentQuestion.CourseId, SmsOption.CourseQuestionAnswer, courseStudentQuestion.StudentUserId);
                        foreach (var user in studentUsers)
                        {
                            ...
                            SmsService.SendSmsMessageWithPostMethod(user.UserName, message, AppConfigProvider.GetSmsUserName(), AppConfigProvider.GetSmsPassword(), AppConfigProvider.GetSmsSenderPhoneNumber());
                        }
                    }
                }
                catch (Exception ex)
