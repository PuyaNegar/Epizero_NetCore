    ...
    public class StudentOnlineExamFinalizeComponent : IDisposable
    {
        ...
        public void Operation(  int OnlineExamStudentId)
        {
            using (var transaction = mainDbContext.Database.BeginTransaction(IsolationLevel.ReadCommitted))
            {
                try
                {
                    var result = studentOnlineExamsRepository.SingleOrDefault(c => c.Id == OnlineExamStudentId  && c.EnterDateTime != null, c => c.OnlineExam);
                    if (result == null)
                    ...
                    if (result.IsFinalized == false)
                    {
                        ...
                        {
                            var model = CorrectQuestions(result.OnlineExamId, result.StudentUserId, result.Id, result.OnlineExam.MaxGrade, result.OnlineExam.HasNegativePoint);
        ...
        List<StudentOnlineExamResultsModel> CorrectQuestions(int onlineExamId, int studentUserId, int onlineExamStudentId, int MaxGrade, bool HasNegativePoint)
        {
            ...
            {
                var questionAnswers = studentOnlineExamQuestionsComponent.Read(onlineExamId, studentUserId , IsShowCorrectAnswer: true);
        ...
        //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        StudentOnlineExamResultsModel CalculateRawScore(StudentOnlineExamResultsModel result, int MaxGrade, bool HasNegativePoint)
        {
