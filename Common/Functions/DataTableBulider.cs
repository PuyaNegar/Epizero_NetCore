...
namespace Common.Functions
{
    public class DataTableBulider<TViewModel> where TViewModel : class
    {
        ...
        {
            var str = "Convert.ToString(" + DataTableRequest.SearchWith + ").Contains(@0)";
            return string.IsNullOrEmpty(DataTableRequest.search.value) ?
                                        ItemSoruce.AsQueryable() :
                                        ItemSoruce.Where(str, DataTableRequest.search.value.CharacterAnalysis().ToLower()).AsQueryable();
        }
        //********************************************************************************************************************
