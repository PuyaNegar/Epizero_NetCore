...
namespace PanelPresentationLayer.Controllers
{
    ...
    public class RoxyFilemanController : Controller
    {
        ...
        public RoxyFilemanController(IWebHostEnvironment env)
        {
            ...
            _systemRootPath = AppConfigProvider.GetAppFilePath(); //env.ContentRootPath;
            _tempPath = _systemRootPath +  "\\uploads\\";
            _filesRootPath = "/uploads";
            _filesRootVirtual = AppConfigProvider.CdnUrl() + "/uploads";
        ...
        {
            _settings = JsonConvert.DeserializeObject<Dictionary<string, string>>(System.IO.File.ReadAllText(_env + "/wwwroot/lib/fileman/conf.json"));
            string langFile = _env + "/wwwroot/lib/fileman/lang/" + GetSetting("LANG") + ".json";
            if (!System.IO.File.Exists(langFile)) langFile = _env + "/wwwroot/lib/fileman/lang/en.json";
